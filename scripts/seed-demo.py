#!/usr/bin/env python3
"""Seed demo data for CommunityOps.
...
def seed(table_name: str, region: str) -> None:
    ...
    for reg_id, name, email, phone, status, pay_status, txn_ref in registrations:
        items.append(
            {
                "PK": ORG_ID,
                "SK": f"EVENT#{EVENT_ID}#REG#{reg_id}",
    ...
    for txn_id, reg_id, amount, status, email, name in payments:
        items.append(
            {
                "PK": ORG_ID,
                "SK": f"EVENT#{EVENT_ID}#PAYMENT#{txn_id}",
    ...
    items.append(
        {
            "PK": ORG_ID,
            "SK": f"EVENT#{EVENT_ID}#CHECKIN#REG-2026-004829",
            "entity_type": "CHECKIN",
            "event_id": EVENT_ID,
    ...
    speakers = [
        ...
        (
            ...
            "INVITED",
            "Scaling Community-Led Developer Programs",
            "TALK",
            False,
        ),
        (
        ...
        (
            ...
            "meera.j@example.com",
            "CONFIRMED",
            "AI in Healthcare: Ethics and Practice",
            "TALK",
            True,
        ),
    ...
    for spk_id, name, email, status, topic, session_type, is_backup in speakers:
        items.append(
            {
                "PK": ORG_ID,
                "SK": f"EVENT#{EVENT_ID}#SPEAKER#{spk_id}",
                "entity_type": "SPEAKER",
                "event_id": EVENT_ID,
                "speaker_id": spk_id,
                "name": name,
    ...
    for team_id, name in teams:
        items.append(
            {
                "PK": ORG_ID,
                "SK": f"EVENT#{EVENT_ID}#TEAM#{team_id}",
                "entity_type": "TEAM",
                "event_id": EVENT_ID,
                "team_id": team_id,
                "name": name,
    ...
    tasks = [
        ...
        (
            "TSK-002",
            "TEAM-marketing",
            "Update social media with venue map",
            "COMPLETED",
            "MEDIUM",
        ...
        (
            "TSK-005",
            "TEAM-speakers",
            "Confirm AV requirements with keynote",
            "OVERDUE",
            "CRITICAL",
            "2026-10-13T12:00:00Z",
            [],
        ...
        (
            "TSK-007",
            "TEAM-volunteers",
            "Assign check-in desk volunteers",
            "COMPLETED",
            "HIGH",
        ...
        (
            "TSK-008",
            "TEAM-venue",
            "Receive printed badges from vendor",
            "PENDING",
            "CRITICAL",
    ...
    for task_id, team_id, title, status, priority, due, deps in tasks:
        items.append(
            {
                "PK": ORG_ID,
                "SK": f"EVENT#{EVENT_ID}#TEAM#{team_id}#TASK#{task_id}",
                "entity_type": "TASK",
                "event_id": EVENT_ID,
                "team_id": team_id,
                "task_id": task_id,
    ...
    items.append(
        {
            "PK": ORG_ID,
            "SK": f"EVENT#{EVENT_ID}#INCIDENT#INC-001",
    ...
    items.append(
        {
            "PK": ORG_ID,
            "SK": f"EVENT#{EVENT_ID}#APPROVAL#APR-001",
    ...
    items.append(
        {
            "PK": ORG_ID,
            "SK": f"EVENT#{EVENT_ID}#APPROVAL#APR-002",
            "entity_type": "APPROVAL",
            "event_id": EVENT_ID,
            "approval_id": "APR-002",
    ...
    print(f"  Event: {EVENT_ID} (DevCon Bengaluru 2026)")  # noqa: T201
    print(f"  Registrations: {len(registrations)}")  # noqa: T201
    print(f"  Payments: {len(payments)}")  # noqa: T201
    print(f"  Speakers: {len(speakers)}")  # noqa: T201
    print(f"  Teams: {len(teams)}")  # noqa: T201
    print(f"  Tasks: {len(tasks)}")  # noqa: T201
    print("  Incidents: 1")  # noqa: T201
    print("  Pending approvals: 2")  # noqa: T201


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Seed CommunityOps demo data")
