 ...
 *
 * By asking who is signed in and comparing that to the identity this build
 * configured as the demo user — see `isDemoIdentity`. Nothing is added to the
 * token, no flag is set at sign-in, and no client-side privilege exists: a demo
 * session is authorized by exactly the same `cognito:groups` rule as any other
 * (requirement 2.7). The check survives a reload and clears itself on sign-out,
 * which an in-memory "the demo button was pressed" flag would not.
 *
 * With no demo credentials configured — the state today (design.md A18) — no
 * session can match, and this renders nothing.
 */

import { useMemo } from "react";

import { getSignedInEmail } from "../auth";
import { isDemoIdentity } from "../lib/demoAccess";
...
export function DemoWorkspaceChip() {
  ...
  /**
   * Read once per session status rather than on every render: the signed-in
   * username cannot change without the session changing, and this reaches into
   * the Cognito SDK's storage to answer.
   */
  const isDemo = useMemo(
    () => status === "authenticated" && isDemoIdentity(getSignedInEmail()),
    [status],
  );
