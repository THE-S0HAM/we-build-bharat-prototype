...
const api = vi.hoisted(() => ({
  ...
  taskWrites: [] as { teamId: string; taskId: string; changes: Record<string, unknown> }[],
}));
...
vi.mock("../taskUpdate", () => ({
  updateTask: (
    ...
    api.taskWrites.push({ teamId, taskId, changes });
...
describe("the team-first board once the contract responds", () => {
  ...
  it("submits only the fields the task endpoint allows, and updates in place (requirement 7.8)", async () => {
    ...
    expect(api.taskWrites).toEqual([
      { teamId: "TEAM-7", taskId: "TSK-4", changes: { status: "BLOCKED" } },
    ]);

    // The card behind the drawer reflects the change without a refetch.
  ...
  it("keeps a team whose tasks could not be read, and says so", async () => {
    api.failingTeams = new Set(["TEAM-42"]);

    renderBoard();

    const card = await screen.findByRole("region", { name: "Platform crew" });
    ...
    // The team that did answer is unaffected (requirement 13.8).
