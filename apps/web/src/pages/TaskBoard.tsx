...
import { getEventTasks, getWorkload, reassignTask, updateTask } from "../api";
...
import type { Task, Team, WorkloadResponse } from "../types";
type TaskUpdate = Partial<Pick<Task, "status" | "priority" | "escalation_level" | "blocked_reason">>;
import {
  assigneeLabel,
  blocksSentence,
  dependsOnSentence,
...
function changesIn(task: Task, draft: TaskDraft): TaskUpdate {
  ...
  if (draft.status === "BLOCKED" && draft.blocked_reason !== (task.blocked_reason ?? "")) {
    changes.blocked_reason = draft.blocked_reason;
  }
...
export function TaskBoard({ eventId }: EventScopedPageProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [directory, taskPage, workloadPage] = await Promise.all([
        loadTeamDirectory(eventId),
      ...
      setWorkload(workloadPage);
      setState({
        kind: "ready",
        teams: directory.teams.map((team) => ({
          team,
          tasks: taskPage.tasks.filter((task) => task.team_id === team.team_id),
          failure: null,
        })),
      });
    } catch (error: unknown) {
  ...
  const assignmentChanged =
    openTask !== null && draft !== null && draft.assigned_to !== openTask.assigned_to;
  const hasGenericChanges = Object.keys(pendingChanges).length > 0;
  const hasChanges = hasGenericChanges || assignmentChanged;

  const submitChanges = useCallback(() => {
    if (openTeam === null || openTask === null || draft === null || !hasChanges) {
      return;
    }
    ...
    void (async () => {
      ...
      try {
        if (assignmentChanged) {
          await reassignTask(eventId, teamId, taskId, {
        ...
        if (hasGenericChanges) {
          await updateTask(eventId, teamId, taskId, pendingChanges);
          mutationAccepted = true;
        }
        await load();
        setSave({ kind: "saved" });
        ...
        <>
          ...
          {workload !== null && (
            <p className="page-section__description">
              {workload.busiest_member === null
                ? "No member workload has been recorded."
                : `${workload.busiest_member.display_name} currently carries ${workload.busiest_member.open_tasks} open tasks. Workload per member comes from the backend.`}
            </p>
          )}
            ...
            <ul className="team-tasks">
              {openTasks.map((task) => (
                <li className="team-task" key={task.task_id}>
                  ...
                  {openTaskId === task.task_id && draft !== null ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="form-field">
                          ...
                          >
                            {STATUS_OPTIONS.filter((status) => status !== "OVERDUE").map((status) => (
                        ...
                        {draft.status === "BLOCKED" && <div className="form-field"><label className="form-label" htmlFor="task-blocked-reason">Blocking reason</label><textarea className="input" id="task-blocked-reason" rows={2} required value={draft.blocked_reason} onChange={(event) => editDraft({ blocked_reason: event.target.value })} /></div>}
                        ...
                        {assignmentChanged && (
                          <div className="form-field">
                            <label className="form-label" htmlFor="task-reassignment-reason">Reassignment reason</label>
                            <textarea
                              className="input"
                              id="task-reassignment-reason"
                              rows={2}
                              required
                      ...
                      <div className="form-actions" role="status">
                        {save.kind === "saved" ? (
                          ...
                          <button
                            ...
                            disabled={save.kind === "saving" || !hasChanges || (draft.status === "BLOCKED" && draft.blocked_reason.trim() === "") || (assignmentChanged && reassignmentReason.trim() === "")}
                          >
                  ...
                  ) : isLeader || task.assigned_to === "" || task.assigned_to === user?.userId ? (
                    <button
                      type="button"
                    ...
                    <p className="team-task__note">Only the assignee or a community leader can update this task.</p>
                  )}
                </li>
              ))}
            </ul>
