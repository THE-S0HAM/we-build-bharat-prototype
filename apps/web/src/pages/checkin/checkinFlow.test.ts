...
describe("a warning is not a failure (requirement 9.6)", () => {
  ...
  it("fails closed for incomplete, duplicate, unknown, or unrelated warning results", () => {
    expect(completionAvailable(null)).toBe(false);
    expect(completionAvailable([])).toBe(false);
    expect(completionAvailable(ALL_PASS.slice(0, 6))).toBe(false);
    expect(completionAvailable([...ALL_PASS.slice(0, 6), ALL_PASS[0]!])).toBe(false);
    expect(
      completionAvailable([
        ...ALL_PASS.slice(0, 6),
        { name: "unexpected_check", status: "PASS", message: "ok" },
      ]),
    ).toBe(false);
    expect(completionAvailable(withCheck("payment_status", "WARN", "Payment delayed"))).toBe(false);
    expect(
      completionAvailable(withCheck("checkin_eligibility", "WARN", "Manual review needed")),
    ).toBe(false);
  });
});
