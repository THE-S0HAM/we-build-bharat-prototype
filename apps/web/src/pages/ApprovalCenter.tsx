...
import { decideApproval, formatInrWithSymbol, getApproval, getApprovals } from "../api";
...
import type { Approval, ApprovalDecisionResponse, BudgetProjection } from "../types";
...
export function ApprovalCenter({ eventId }: EventScopedPageProps) {
  ...
  const [projections, setProjections] = useState<Readonly<Record<string, BudgetProjection | null>>>({});
  const [projectionFailure, setProjectionFailure] = useState<unknown>(null);
    ...
    (onFailure: ((error: unknown) => void) | null) => {
      ...
      getApprovals(eventId).then(
        (response) => {
          ...
          const pending = response.approvals.filter((approval) => approval.status === "PENDING");
          setQueue(pending);
          if (typeof getApproval === "function") {
            void Promise.all(pending.map((approval) => getApproval(eventId, approval.approval_id))).then((details) => {
              setProjections(Object.fromEntries(details.map((detail) => [detail.approval.approval_id, detail.budget_projection])));
              setProjectionFailure(null);
            }, (error: unknown) => { setProjections({}); setProjectionFailure(report(error)); });
          }
          setLoading(false);
        },
        (error: unknown) => {
  ...
  const decide = useCallback(
    async (approval: Approval, request: DecisionRequest): Promise<DecisionOutcome> => {
      let result: ApprovalDecisionResponse;
      try {
      ...
      setBudgetResult(result.budget ? `Authoritative remaining budget: ${formatInrWithSymbol(result.budget.remaining)}.` : result.message);
  ...
  return (
    <div className="page approvals">
      ...
      {budgetResult && <p className="form-result" role="status">{budgetResult}</p>}
      {projectionFailure !== null && <ApiErrorState error={projectionFailure} onRetry={load} />}
        ...
        <section className="approvals__queue" aria-label={QUEUE_LABEL}>
          {cards.map((approval) => (
            <div key={approval.approval_id}>
              {projections[approval.approval_id] ? <p className="page-section__description">Backend projection: {projections[approval.approval_id]?.impact_summary}</p> : null}
              <ApprovalCard
                approval={approval}
