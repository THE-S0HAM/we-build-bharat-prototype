...
describe("federated provider allowlist", () => {
  it("omits the provider block and its divider when no provider is configured", () => {
    ...
    // Leaving the form as the complete sign-in surface: one button, and it is
    // the primary action (requirement 12.11).
    expect(screen.getAllByRole("button")).toHaveLength(1);
    expect(submitButton()).toBeInTheDocument();
  });

  it("renders one button per named provider once the hosted UI exists", () => {
    ...
    // The form is still the same form; providers are an addition, not a mode.
    expect(submitButton()).toBeInTheDocument();
  });
...
describe("screen structure", () => {
  ...
  it("offers nothing that is not backed by something real", () => {
    ...
    // "Try Demo Account" belongs to task 2.6 and its configuration is absent.
