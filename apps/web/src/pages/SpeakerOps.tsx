...
import { draftSpeakerFollowup, getAuditLog, getSpeakers, updateSpeaker } from "../api";
...
import type { AuditEvent, FollowupDraft, Speaker } from "../types";
type SpeakerUpdate = Partial<Pick<Speaker, "status" | "travel_required" | "accommodation_required" | "is_backup">>;
...
export function SpeakerOps({ eventId }: EventScopedPageProps) {
  ...
  const canManageSpeakers = user?.role === "LEADER";
  ...
  const [followupFailure, setFollowupFailure] = useState<unknown>(null);
  ...
  const prepareFollowup = useCallback(() => {
    if (openSpeaker === null || draftingFollowup) return;
    setDraftingFollowup(true);
    setFollowupFailure(null);
    draftSpeakerFollowup(eventId, openSpeaker.speaker_id).then(
      (result) => { setFollowupDraft(result); setDraftingFollowup(false); },
      (error: unknown) => { setFollowupFailure(report(error)); setDraftingFollowup(false); },
    );
  }, [draftingFollowup, eventId, openSpeaker, report]);
  ...
  const columns = useMemo<readonly DataTableColumn<Speaker>[]>(
    () => [
      ...
      { key: "topic", header: "Topic", cell: (speaker) => speaker.topic },
      {
        key: "followups",
        header: "Follow-ups",
        cell: (speaker) => speaker.followup_count,
      },
      {
        key: "status",
      ...
      {openSpeaker !== null && draft !== null && (
        ...
        >
          <dl className="detail-list">
            ...
            {canManageSpeakers && <section className="drawer-section" aria-labelledby="speaker-followup-heading">
            <h3 className="drawer-heading" id="speaker-followup-heading">Prepare a follow-up</h3>
            <p className="drawer-description">CommunityOps can prepare the next message. It will not send it; sending remains approval-aware and requires the backend workflow.</p>
            {followupDraft === null ? (
              <button type="button" className="btn" onClick={prepareFollowup} disabled={draftingFollowup}>
                {draftingFollowup ? "Preparing…" : "Prepare follow-up draft"}
              </button>
            ) : (
              <div className="ops-result" role="status">
                <strong>Prepared, not sent.</strong>
                <p>{followupDraft.draft}</p>
                <p>{followupDraft.message}</p>
              </div>
            )}
            {followupFailure !== null && <ApiErrorState error={followupFailure} context="action" />}
          </section>}
          ...
          >
            ...
            <fieldset className="form-fields" disabled={save.kind === "saving"}>
              ...
              <div className="form-field">
                ...
                >
                  {STATUS_OPTIONS.filter((status) => status !== "CONFIRMED" || status === openSpeaker.status).map((status) => (
