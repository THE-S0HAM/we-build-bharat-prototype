...
const cognito = vi.hoisted(() => ({
  ...
  user: {
    ...
    organizations: ["ORG-primary", "ORG-active"],
    isDemo: false,
  },
}));
...
vi.mock("./api", async (importOriginal) => {
  ...
  return { ...actual, getEvents: api.getEvents, getOrganizationContext: api.getOrganizationContext };
});
...
beforeEach(() => {
  ...
  cognito.user.organizations = ["ORG-primary", "ORG-active"];
  api.getEvents.mockResolvedValue({ events: [], count: 0 });
  api.getOrganizationContext.mockResolvedValue({
    organizationId: "ORG-active",
    selectable: ["ORG-primary", "ORG-active"],
    source: "url",
  });
});
...
describe("organization authorization failures", () => {
  it("denies a valid identity with no organization without mounting or displaying a fallback tenant", async () => {
    cognito.user.organizations = [];

    renderConsole(INERT_PROTECTED_PATH);

    expect(await screen.findByRole("heading", { name: "Access unavailable" })).toBeInTheDocument();
    expect(screen.getByText(/not assigned to a CommunityOps organization/i)).toBeInTheDocument();
    expect(screen.queryByText("ORG-active")).not.toBeInTheDocument();
    expect(api.getOrganizationContext).not.toHaveBeenCalled();
    expect(api.getEvents).not.toHaveBeenCalled();
  });

  it("handles organization resolution failures as a stable denied state", async () => {
    api.getOrganizationContext.mockRejectedValueOnce(new Error("resolution failed"));
...
describe("active organization presentation", () => {
  it("shows the same membership-checked organization resolved for API requests", async () => {
    renderConsole(INERT_PROTECTED_PATH);
    expect(await screen.findByText("ORG-active")).toBeInTheDocument();
    expect(screen.queryByText("ORG-primary")).not.toBeInTheDocument();
  });
});
