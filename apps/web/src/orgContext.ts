...
export function selectableOrganizations(idToken: string | null): string[] {
  return readMemberOrganizations(idToken);
}
...
export type ResolveOrganizationOptions = {
  ...
  /**
   * Used only when the token names no organization at all — no session, or a
   * token with no `cognito:groups` claim. See `resolveOrganization`.
   */
