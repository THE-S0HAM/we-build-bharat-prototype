...
export const CAPABILITIES: NavCapabilities = {
  // AttendeeOps: `GET /events/{eventId}/attendees` does not exist and no
  // attendee records are seeded, so the entry and the route are both absent
  // (design.md A6, §8.4). Phase J turns this on together with the real client.
  attendeeOps: isOn(import.meta.env.VITE_CAPABILITY_ATTENDEE_OPS),
};
