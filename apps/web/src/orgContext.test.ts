...
describe("role groups are not tenants", () => {
  it("selects only ORG-prefixed memberships from a mixed Cognito group claim", () => {
    const token = tokenFor({ sub: "user-2", "cognito:groups": ["TEAM_MEMBER", "ORG-community", "LEADER"] });
    expect(selectableOrganizations(token)).toEqual(["ORG-community"]);
    expect(resolveOrganization({ idToken: token, fallbackOrganizationId: FALLBACK, search: "?organization_id=LEADER" })).toMatchObject({ organizationId: "ORG-community", source: "token" });
  });
});
