...
describe("no component gates access on a client-side role or permission check", () => {
  /**
   * Identifiers that only exist to make an access decision in the browser.
   * `capabilities.ts` is the deliberate contrast: a capability flag says whether
   * a backend contract exists, and turning it on grants nothing — so it is not
   * on this list, while anything phrased as a role or a permission is.
   */
  const AUTHORIZATION_IN_THE_BROWSER: readonly RegExp[] = [
    /\bhasRole\b/,
    /\bhasPermission\b/,
    /\bhasAnyRole\b/,
    /\bisAdmin\b/,
  ...
  it("names no role or permission check in any shipped module", () => {
    // A guard over an empty file list would pass without checking anything.
    expect(SOURCES.length).toBeGreaterThan(0);

    const offences = SOURCES.flatMap(({ path, text }) =>
      AUTHORIZATION_IN_THE_BROWSER.filter((pattern) => pattern.test(code(text))).map(
  ...
  it("holds no permission or role on the session", () => {
    expect(sourceOf("session/sessionContext.ts")).not.toMatch(
      /\b(?:role|roles|permission|permissions|scopes)\b/,
    );
  });
});
